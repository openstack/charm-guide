...
* Informational notices
   ...
   * `Cinder charm: Change the method used to configure a clustered Volume service`_
...
Cinder charm: Change the method used to configure a clustered Volume service
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The Cinder charm changes the configuration of the Volume service to adhere to
the recommended upstream (see `High Availability Cinder-Volume`_) of using the
"cluster" configuration key instead of "host" for storage services that
support Active-Active setup.

With this change /etc/cinder/cinder.conf for environments running OpenStack
Ocata or newer will see the "cluster" key set when the configured storage
backend is stateless (e.g. Ceph).
...
Purestorage: Active-Active configuration is not currently supported
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

When a volume is created in OpenStack releases from Pike to Ussuri (included)
it will go into error state and print the following message in the logs:
"Active-Active configuration is not currently supported". this happens
because the Volume service was configured to use the configuration key
"cluster" which demands from the driver to claim Active-Active HA support
explicitly and this feature flag was enabled for Purestorage in the Victoria
release.

Environments running Purestorage are discouraged from upgrading until the
issue tracked in bug `LP #1947702`_ is fixed.
