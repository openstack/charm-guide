...
The vault charm now supports actions ``pause`` and ``resume`` to
respectively stop and start the Vault process on units.

For further details consult appendix `Vault`_ in the `OpenStack
